/*
  ...
  Known issues:
      If sched.h is not installed (rare), or it does not give the right results (e.g. always zero),
      try to compile with -DNOSCHED flag (so the code will get the CPU id by parsing the output of '/proc/self/stat')
